...
loader.load('https://threejs.org/examples/fonts/helvetiker_regular.typeface.json', function (font) {
    const textGeometry = new TextGeometry('Current Projects', {
        font: font,
        size: 1,
        height: 0.1,
        curveSegments: 1,
    ...
    window.addEventListener('click', function (event) {
        ...
        const intersects = raycaster.intersectObject(textMesh);
        if (intersects.length > 0) {
            window.location.href = 'projects.html'; // Redirect to the new site
        }
    });
});
...
loader3.load('https://threejs.org/examples/fonts/helvetiker_regular.typeface.json', function (font) {
    ...
    window.addEventListener('click', function (event) {
        ...
        const intersects = raycaster.intersectObject(textMesh);
        if (intersects.length > 0) {
            window.location.href = 'mailto:mail@max-dufner.me'; // Open mail client
        }
    });
});
